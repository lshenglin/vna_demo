function [phi, range,phase1,phase2,MY_PHI] = extVNAdata(fname, vnaCfg, antCfg, pkCfg, Nup, Npath, mode, ang_val, valid_range)
% MY_PHI is phase difference between phase1 and phase2, zero at orthogonal
% facing position.
if nargin == 4
    mode = 'csv';
    ang_val = 0;
...
% extract time and frequency response
[hEst_t, hEst_f, delay, PDP, ang, fs] = chanEstVNA(fname, pkCfg, Nup, mode, ang_val);
...
%     	range(cnt) = 3e8/5.5e9*(delay(idx, 1, 1)+delay(idx, 1, 2))/2/Nup;
        range(cnt) = 3e8/fs*(delay(idx, 1, 1)+delay(idx, 1, 2))/2;
    	
    	phase1(cnt) = round(angle(PDP(idx, 1, 2))/pi*180+180);%in degrees
    	phase2(cnt) = round(angle(PDP(idx, 1, 1))/pi*180+180);
    end
    fprintf(1, 'ang = %.1f: Estimated: phi = %.1f (%.1f), theta = %.1f\n', lp, phi(cnt), phi_acos(cnt), theta(cnt));
    cnt = cnt + 1;
end

% phi_shift = [phi_phase(91:180) phi_phase(1:90)];
% phi_ls = unwrap(phi_shift(valid_range));
% phi_ls = 180/pi*phi_ls.';
% ang_plot_vec_shift = unwrap([ang_plot_vec(91:180); ang_plot_vec(1:90)]/180*pi)*180/pi-360;
